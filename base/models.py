...
class Profile(models.Model):
    ...
    profile_bio= models.CharField(max_length=400, null=True, blank=True)
    ...
    linkedln_link= models.CharField(max_length=100, null=True, blank=True)
...
#autimatically create profile when user signup
@receiver(post_save, sender= User)
def create_profile(sender, instance, created, **kwargs):
    if created:
        user_profile = Profile(user= instance)
        user_profile.save()
     #make new user to automatically follow themself when they signup
        user_profile.follows.set([instance.profile.id])
        user_profile.save()   
#post_save.connect(create_profile, sender=User) this is used when we don't import reciever